  ...
  return (
    <FormControl fullWidth>
      ...
      <Select
        id="demo-simple-select"
        label={title}
        defaultValue={typeof tempState === "string" ? tempState : tempState.name}
        // defaultValue={tempState}
        onChange={handleChange}
      >
        {data.length > 0 &&
          data.map((dataItem, index) => {
            return (
              <MenuItem
                ...
                value={typeof dataItem === "string" ? dataItem : dataItem.name}
              >
                {typeof dataItem === "string" ? dataItem : dataItem.name}
                {/* {dataItem} */}
              </MenuItem>
            );
          })}
